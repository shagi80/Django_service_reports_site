...
def UploadProductionFiles(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
                    ...
                    try:
                        with open(path, "r") as fileData:
                            for line in fileData:
                                ...
                                date_time = make_aware(datetime.datetime.strptime(
                                    string_dict[0], '%y.%m.%d %H:%M:%S'))
                                # date_time = make_aware(datetime.datetime.strptime(string_dict[0], '%Y-%m-%d %H:%M:%S'))
                                code = ''.join(
                                    filter(str.isalnum, string_dict[2]))
                                try:
                                    if shop_char == FACTORY_NOVATEK:
                                        model = Models.objects.get(
                                            product_id=product_id, east_id=int(code[:2]))
                                    else:
                                        model = Models.objects.get(
                                            product_id=product_id, east_id=int(code[:2]))
            ...
            if counter:
                messages.success(
                    request, f'{counter} записей успешно загружено !')
            else:
